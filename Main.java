...
class Player {
	...
	public Player()throws Exception{
		firstInput();//初回inputのみ
        firstProcess();//工場の生成、コネクションを生成・最短経路距離の算出・最短経路距離の昇順sortしてるだけ
        while (true) {
            loopInput();//loop毎の入力
            if(nowTurn==0)firstTurnProcess();//playerId決めと敵最短の工場決め
    ...
    void firstProcess(){//最初に１回しかやらない、複数回やる必要がない処理はここに書くべき
    	for(int i=0; i<factoryCount;i++){
        	factoryMap.put(i, new Factory(i));
        }
        for(Factory f: factoryMap.values()){
        	f.generateConnection();//この工場が持つべきコネクションをまとめて保存するだけ
        }

        for(Factory f: factoryMap.values()){//最短経路距離をもとめるループ
        	for(Connection c: f.connectionList){
        		int totalDist = 0;
        		int fromId = f.id;
        		/*
        		while(true){
        			int toId = calcShortestRoute(fromId,c.factory.id, c.dist-totalDist);//最短経路における次の行き先をもとめる
	      	 	 	totalDist += calcShortestRouteDist(fromId,c.factory.id, c.dist-totalDist);//最短経路における次の行き先までの距離を求める
	      	 	 	if(toId == c.factory.id)break;
	      	 	 	fromId = toId;
	      	 	}*/
	      	 	totalDist += calcShortestRouteDist(fromId,c.factory.id, c.dist-totalDist);//最短経路における次の行き先までの距離を求める
        ...
        for(Factory f: factoryMap.values()){
        	f.sortConnection();//最短経路距離の昇順にsort
    ...
    void loopInput(){
        ...
        for (int i = 0; i < entityCount; i++) {
            ...
            case 'F':
            	...
            	factory.inputState(tmp);//ただ代入しているだけ
            	break;
            case 'T':
    ...
    //本当はfirstProcessでやりたかったが、１度loopInputで得た情報が無いと出来ない初回限定処理
    public void firstTurnProcess(){
    	...
    	for(Factory f: factoryMap.values()){
			...
			f.calcNearEnemy();//最も近くの敵工場のdistをnearEnemyDistに保存するだけ
			f.calcNearMyFactory();//最も近くの味方工場のdistをnearMyFactoryDistに保存するだけ
	...
	String mainProcess()throws Exception{
        ...
        beforeProcess();//justボム判定、工場状態の更新、敵最短の自分工場の決定

        calcTree(factoryTree);//各工場のhelp優先度を決めて、それ順に並び替える
        ...
        debug("");System.err.print("help2:"); for(Factory f: tmp2)System.err.print("["+f.id+","+f.calcHelp2()+"] "); debug("");//工場のhelp要請数を優先度順に表示

    	ans += decideBomb();//ボム処理
    	ans += escapeBombProcess();//ボム処理
        for(Factory f: tmp2){
	...
	//ループ毎にメイン処理をする前にやっておきたい前処理はここに
	public void beforeProcess(){
		for(Factory f: factoryMap.values()){
			f.gatherComeTroop();//comeTroopListを更新しているだけ（競合する可能性のある操作は無し）
			f.sortComeTroop();//comeTroopListをremainTimeの昇順にsortしているだけ（comeTroopListが更新されたあとなら競合無し）
			f.gatherBomb();//bombListを更新しているだけ（競合する可能性のある操作は無し）
			f.sortConnectionProd();//connectionSortProdListを接続先の工場のoriProd降順にsortしてるだけ（初回にconnectionができてれば競合なし）
			f.judgeNextTurnOwn();//ニュートラル工場のみに対し次ターンで所有できるかを判定（comeTroopができていれば競合なし）
			f.calcNearEnemy();
			f.calcNearMyFactory();
			f.calcAveMyFactory();
			f.nearestEnemy = 0;
		...
		int min = 999;
		int minId =-1;
		for(Factory f: factoryMap.values()){//敵最短の自工場を決める
	...
	public void calcTree(TreeMap<Double, Factory> tree){
		//資源奪取、INC、防衛と様々な意味合いがあるので、それぞれの専用calcTree処理するのはどう？
    	 for(Factory f: factoryMap.values()){
    		 double score = 0;
    		 int subDist = f.nearMyFactoryDist - f.nearEnemyDist;//敵のほうが近ければ正の数になる
    		 int helpNum = f.calcHelp2();//未来において敵に占領されることが決まっているなら、必要なhelp数を返す
    		 switch(f.owner){
    		 case 1:
    		 ...
    		 case 0:
    			 ...
    			 //score += -subDist;
    			 score += -f.nearMyFactoryDist;//nearとの比較すべき
    			 score += f.oriProd*5;
    			 score += -f.cybNum;//prodとの真の優先度を考えるべき
    			 break;
    		 case -1:

    		 	int nowTurn = 1;//これ全体もシミュ化すべき、敵のcyb移動状態も考慮した
